...
- **対応バージョン**:　1.21.11
- **前提プラグイン**:　CrackShotPlugin（URL=https://www.spigotmc.org/resources/crackshot-guns.48301/）
- **リソースパック**: プラグイン本体としては**不要**（カスタムテクスチャ等は使用せずバニラの見た目で動作）。ただし銃プラグインを導入する場合は、そちら側でリソースパックが必要になることがあります(銃プラグイン側の仕様に依存するため本プラグインのコード上では確認できません)。
- **チーム**: 赤（red）／青（blue）の2チーム固定です。

## 導入方法

1. メインプラグインDomination-1.0.jarをこのGithubからダウンロードし、前提プラグインの**Crackshot.jar**をpluginsフォルダに配置します。
2. Github内にあるCrackShotフォルダをpluginsフォルダに配置します。
3. サーバーを起動します。
